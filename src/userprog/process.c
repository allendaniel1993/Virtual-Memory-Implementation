...
//change-03
 // Initialize the supplement page table for the current thread
 init_sup_page_t(&thread_current()->sup_page_table);
//change-03
...
process_exit (void)
{
 ...
 // Remove the supplementary page table of the current thread
    destroy_page_t(&cur->sup_page_table);
              ...
              uint32_t read_bytes, uint32_t zero_bytes, bool writable) 
{
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
	...
	//Lazy Loading - causes issues so commenting 

//	  page_entry_ptr_t page_c_new =NULL;
//	  page_c_new = create_page_entry(file, ofs, upage, read_bytes, zero_bytes, writable);        
//		if ( page_c_new == NULL)//If the page is present in page table is should return false
//			{
//				return false;
//            		}
//					          
//		if (page_read_bytes < PGSIZE && page_zero_bytes < PGSIZE)//If the read data has size lesser than page size it is read from file
//			{
//				load_page_file(page_c_new);
//			}       
...
{
 uint8_ptr_t kpage = NULL, upage = ((uint8_t *) PHYS_BASE) - PGSIZE;
 bool success = false;
	     
 kpage = build_stack(upage);    
 if (kpage != NULL)
 {
